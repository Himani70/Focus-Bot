...
#slack_token = "xoxb-792834100421-803926781638-6tFjHdoCDPzYBr4OFMug7qCW"
...
jira = JIRA(basic_auth=(tokens.get("userId"), tokens.get("userToken")), options={'server': tokens.get("serverEndPoint")})
channelId = None
...
def get_user(user_id, test=False):
    if test:
        return user_id
...
def GetDataFromDb(userid):
    ...
    if row_count == 0:
       response = 'No progress for the week!'
       slack_client.api_call("chat.postMessage", channel=channelId, text=response)
    else:    
...
def processMessage(data, test=False):
    # if "test" in tokens:
    #     #  slack_client
    #     # global jira
    #     slack_client = SlackClient(tokens.get("slack_bot_token"))
    ...
    #user = data['user']
    # Describe commands
    if(tokens.get("focusBot") in data['text']):
        # print('hey')
        global channelId
    #slackEvent = request.get_json().get('event')

        type_of_event = data['type']
        text = data['text']
        # print(text)
        userID = data['user']
        ...
        if type_of_event == "message" and "list tasks" in text.lower() and userID != None:
            ...
            in_progress_issues = jira.search_issues(
                "assignee=%s and status='In Progress'" % get_user(userID, test))
            response = []
            to_do_issues = jira.search_issues(
                "assignee=%s and status='To Do'" % get_user(userID, test))
        ...
        #Command : Create task <project-key> <issue-type> <issue-name> <issue-description> <number-of-points>
            ...
            if len(params) < 8 :
                ...
                issue_dict = {
                    ...
                    "assignee": {"name": get_user(userID, test)}
                }
                #issuetype={'name': 'Task'}
                ...
                response = validateNewIssue(issue_dict,get_user(userID, test))
                if not response:
            ...
            #slack_client.api_call("chat.postMessage", channel=channelId, text=new_issue)
        ...
        # eg: Start Task {TaskId} {Session Time} {Short Break Time} {Long Break Time}
            ...
            cmd = text.split()[3:]
            user = get_user(userID, test)
            activetask = getFromUserMetadata(user, "activetask")
            if activetask != "-1":
                response = "You already have an active pomodoro task"
                slack_client.api_call("chat.postMessage", channel=channelId, text=response)
            else:
                flag, response = validateStartTask(cmd, user)
                if not flag:
                    slack_client.api_call("chat.postMessage", channel=channelId, text=response)
                else:
                    req = {}
        ...
        #sad path - No active sprint/ no tasks
            ...
            t = threading.Thread(target=GetDataFromDb, args=(get_user(userID, test) , ))
            t.start()
    else:
        response = 'Invalid command format'
            
        
    r = Response(response=response, status=200, mimetype="text/plain")
    r.headers["Content-Type"] = "text/plain"
    return r
...
def submit_button_task(data):
    ...
    if data["actions"][0]["type"] == "button":  # stop task
        ...
        # get epoch from DB
        day = curr_datetime.tm_wday  # 0 to 6
        if day > 4:
            return
...
if __name__ == "__main__":
    if slack_client.rtm_connect():
        while slack_client.server.connected is True:
            for data in slack_client.rtm_read():
                # channelId = data['channel']
                if "type" in data and "text" in data and data["type"] == "message":
                    processMessage(data)
    else:
